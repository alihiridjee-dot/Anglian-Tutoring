...
export function LiveForm({ userId, taxonomy }: LiveFormProps) {
  ...
  const generateTeamsLink = (e: React.MouseEvent) => {
    e.preventDefault();
    const randomMeetingId = Math.random().toString(36).substring(2, 10) + "-" + Math.random().toString(36).substring(2, 6);
    ...
    if (broadcastWhatsApp) {
      ...
      try {
        ...
        toast.success("Scheduled & WhatsApp invite copied!", {
          description: "We saved the lesson and copied the pre-formatted WhatsApp invite text to your clipboard. Paste it directly in your WhatsApp group chat!",
          duration: 6000
        });
      } catch {
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="p-3 bg-secondary/50 rounded-lg border border-border space-y-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Broadcast Notifications</span>
        </div>
