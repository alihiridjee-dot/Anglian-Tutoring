...
import { SUBJECT_TEXT, subjectLabel } from "@/lib/curriculum/subjectTheme";
        ...
        <div className="space-y-4">
          {items.map((f) => (
            <div key={f.id} className="bg-secondary border border-border rounded-xl p-4">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed italic">"{f.feedback}"</p>
              {(f.grade || f.scorePct != null) && (
                <div className="mt-3 flex gap-3 items-center border-t border-border pt-3 text-[10px] text-muted-foreground">
