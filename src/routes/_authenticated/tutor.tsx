...
import { PlayCircle, CalendarClock, ClipboardList, Upload } from "lucide-react";
...
function Tutor() {
  const { isTutor, loading, userId } = useRoles();
  ...
  const tabs: { k: Kind; label: string; icon: typeof PlayCircle }[] = [
  ...
  return (
    <AppLayout title="Tutor Studio">
      ...
      <div className="max-w-2xl rounded-2xl bg-card border border-border p-6">
        {kind === "video" && <VideoForm userId={userId!} taxonomy={taxonomy} />}
        {kind === "live_session" && <LiveForm userId={userId!} taxonomy={taxonomy} />}
        {kind === "homework" && <HomeworkForm userId={userId!} taxonomy={taxonomy} />}
        {kind === "download" && <DownloadForm userId={userId!} taxonomy={taxonomy} />}
      </div>
    </AppLayout>
  );
}
