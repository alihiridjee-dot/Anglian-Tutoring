...
test("UK Monday keys do not follow the viewer's timezone", () => {
  ...
  for (const TZ of ["UTC", "America/New_York", "Asia/Tokyo"]) {
    const result = Bun.spawnSync([process.execPath, "-e", `import {currentWeekKey,weekKeyToDate} from './src/lib/week.ts'; console.log(currentWeekKey(new Date('2026-09-06T23:30:00Z')),weekKeyToDate('2026-09-07').toISOString())`], { env: { ...process.env, TZ } });
...
test("calendar weeks retain midnight through both BST transitions", () => {
  for (const [key, hours] of [["2026-03-23", 167], ["2026-10-19", 169]] as const) {
    const start = weekKeyToDate(key), next = addWeeks(start, 1);
...
test("invalid date keys fail rather than rolling into another month", () => {
  for (const key of ["2026-02-30", "2026-13-01", "garbage"]) expect(() => weekKeyToDate(key)).toThrow();
});
