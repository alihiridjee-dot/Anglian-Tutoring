...
test("concurrent roadmap and memory views share one course evidence read", async () => {
  ...
  try {
    await Promise.all([client.fetchQuery(roadmapQuery(client, p)), client.fetchQuery(roadmapQuery(client, p)), client.fetchQuery(memoryQuery(client, p))]);
  ...
  } finally { progress.mockRestore(); roadmap.mockRestore(); client.clear(); }
});
