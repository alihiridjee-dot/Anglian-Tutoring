...
export const courseKey = (p: PlannerCourse) =>
  [...plannerKey(p.studentId), p.subject, p.board, p.level] as const;
export const progressQuery = (p: PlannerCourse) =>
  queryOptions({
    queryKey: [...courseKey(p), "progress", currentWeekKey()],
    queryFn: () => ScheduleDAL.getTopicProgress(p),
    staleTime: 30_000,
  });
export const roadmapQuery = (client: QueryClient, p: PlannerCourse, projectOnly = false) =>
  queryOptions({
...
export const memoryQuery = (client: QueryClient, p: PlannerCourse) =>
  queryOptions({
    queryKey: [...courseKey(p), "memory", currentWeekKey()],
    queryFn: async () =>
      ScheduleDAL.getMemoryStats({ ...p, progress: await client.fetchQuery(progressQuery(p)) }),
    staleTime: 30_000,
  });
