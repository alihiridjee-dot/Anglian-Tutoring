...
11. **Test on the test account, never on the demo.** The `/demo/*` showcase exists for sales only: it has no session and renders fixtures, so it never touches the route guard, RLS, the paywall, a real query or a real write. End-to-end testing is done signed in as the dedicated test student `123@123.com`, which has no active card and cannot be charged. It is a real student on the live project, so its writes are real — submitting a quiz files an attempt, and handing in homework is final. Its credentials are not kept in this repository: ask Ali, and never commit or paste them anywhere. See `docs/AUTHENTICATION.md`.
